...
Trellis Kit 用于安装 Trellis 工作流规则、Claude 自定义命令，以及可选的 Review Handoff / 外部审查辅助工具。

它是一个无运行时依赖的小型 Node.js CLI 包。它不会替代 Trellis、Claude Code 或 Codex CLI，只会复制可纳入版本管理的项目文件，让 Claude Code 按 Trellis 工作流实现代码，并可选择生成 Review Handoff 用于手动外部审查。可选 Codex 审查仍由用户手动决定和执行。
