...
- Claude Code must run the installed OS-native review script after the implementation commit for every `/dev` implementation request: `.trellis/spec/scripts/codex-review.sh .trellis/tasks/<task>` on macOS/Linux or `.\.trellis\spec\scripts\codex-review.ps1 .trellis/tasks/<task>` on Windows PowerShell.
- Claude Code must fix P0/P1 issues by default and write `.trellis/tasks/<task>/reviews/claude-fix-notes.md`.
- Claude Code must run the installed OS-native re-review script after P0/P1 fixes: `.trellis/spec/scripts/codex-rereview.sh .trellis/tasks/<task>` on macOS/Linux or `.\.trellis\spec\scripts\codex-rereview.ps1 .trellis/tasks/<task>` on Windows PowerShell.
...
5. Commit locally.
6. Run the installed OS-native review script (`.trellis/spec/scripts/codex-review.sh .trellis/tasks/<task>` on macOS/Linux or `.\.trellis\spec\scripts\codex-review.ps1 .trellis/tasks/<task>` on Windows PowerShell).
7. Fix only P0/P1 issues by default.
8. Run the installed OS-native re-review script (`.trellis/spec/scripts/codex-rereview.sh .trellis/tasks/<task>` on macOS/Linux or `.\.trellis\spec\scripts\codex-rereview.ps1 .trellis/tasks/<task>` on Windows PowerShell) when fixes were made.
