...
2. 使用 `/review` 生成 `review-brief.md`，并通过 `trellis channel` 运行 Codex check worker。
3. `/review` 保存 Codex 输出到 `.trellis/tasks/<task>/review/codex-review.md` 或编号文件。
4. 使用 `/review-fix` 修复 Blocking / Should Fix findings，并生成 `review-fix-summary.md`。
5. 使用 `/review --rereview` 基于上次 review、fix summary 和本次 diff 复审。
...
不要把直接读取 `events.jsonl` 作为主要结果读取方式，除非是最后排障手段。
